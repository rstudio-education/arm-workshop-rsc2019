...
### .fancy[.center[`r emo::ji("computer")` Local]]
...
### .fancy[.center[`r emo::ji("cloud")` Cloud or `r emo::ji("computer")` Local]]
...
.right-column[
# .fancy[05:00 minutes]
### Orient yourself to your site, and the files in it!

Ideas:

+ Where is the `.md` file for the blog post .bright["Creating A New Theme"]?
+ Where is the `.jpg` file you see in the .bright[hex image] stored? What about the featured images for each blog post?
+ Where is the `.md` file you see under .bright[About]? (i.e., "Hugo is a static site engine written in Go.")
+ Where are the `.md` files you see under .bright[Itemized]? (i.e., fancy apps 1-4)
...
+ ~~Orient ourselves to our site and its files~~ [`r emo::ji("key")`](#yourturn-1)

+ Edit the `config.toml` [`r emo::ji("key")`](#block1)

+ Add content [`r emo::ji("key")`](#block2)
...
Mouse over the `static/img/main/` and see `logo.jpg`? We can replace that file, or make new file. I'll replace mine with [this picture](https://en.wikipedia.org/wiki/Ada_Lovelace).
...
.bright[.fancy[Stretch:]] All possible Hugo configuration variables are [listed here](https://gohugo.io/getting-started/configuration/).
...
```{r echo = FALSE, out.width='70%'}
knitr::include_graphics("img/blogdown-newcontent-post-nobundle.png")
```
...
```{r echo = FALSE, out.width='70%'}
knitr::include_graphics("img/blogdown-newcontent-post-bundled.png")
```
...
1. Use `usethis` package to `edit_r_profile` to enable bundles (and use title case option too!)

1. Restart your R session, serve your site again.
...
.bright[.fancy[Stretch:]] In `content/blog/` there is the sample `.Rmd` blog post titled "Hello R Markdown". Can you find it on the site? Why or why not? Can you spot the problem?
